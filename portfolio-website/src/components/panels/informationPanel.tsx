...
export const InfoPanel: FC = () => {
    ...
    <div id="info" className="h-screen flex justify-center bg-slate-900">
        ...
        <div className="py-5 pl-10 grid grid-flow-row gap-2 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {me.links.map(e => <span className="flex"><a target="_blank" href={e.url}>{e.icon ? <img src={e.icon} className="scale-80"></img> : e.name}</a></span>)}
            <span><a target="_blank" href={"mailto: " + me.email}><img src="/assets/email.svg" className="scale-80"></img></a></span>
        </div>
